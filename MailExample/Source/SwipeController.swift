...
protocol SwipeControllerDelegate: class {
    ...
    func swipeController(_ controller: SwipeController, editActionsForSwipeableFor orientation: SwipeActionsOrientation, point: CGPoint) -> [SwipeAction]?
    
    func swipeController(_ controller: SwipeController, editActionsOptionsForSwipeableFor orientation: SwipeActionsOrientation, point: CGPoint) -> SwipeOptions
...
class SwipeController: NSObject {
    ...
    @objc func handlePan(gesture: UIPanGestureRecognizer) {
        ...
        case .began:
            ...
            if swipeable.state == .center || swipeable.state == .animatingToCenter {
                ...
                showActionsView(for: orientation, point: gesture.location(in: self.scrollView))
            }
        case .changed:
    ...
    @discardableResult
    func showActionsView(for orientation: SwipeActionsOrientation, point: CGPoint) -> Bool {
        guard let actions = delegate?.swipeController(self, editActionsForSwipeableFor: orientation, point: point), actions.count > 0 else { return false }
        guard let swipeable = self.swipeable else { return false }
        
        originalLayoutMargins = swipeable.layoutMargins
        
        configureActionsView(with: actions, for: orientation, point: point)
    ...
    func configureActionsView(with actions: [SwipeAction], for orientation: SwipeActionsOrientation, point: CGPoint) {
        ...
        let options = delegate?.swipeController(self, editActionsOptionsForSwipeableFor: orientation, point: point) ?? SwipeOptions()
...
extension SwipeController: SwipeActionsViewDelegate {
    ...
    func setSwipeOffset(_ offset: CGFloat, animated: Bool = true, completion: ((Bool) -> Void)? = nil) {
        ...
        if swipeable.state != targetState {
            guard showActionsView(for: orientation, point: .zero) else { return }
