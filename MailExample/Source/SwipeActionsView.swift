...
class SwipeActionsView: UIView {
    ...
    func addButtons(for actions: [SwipeAction], withMaximum size: CGSize, contentEdgeInsets: UIEdgeInsets) -> [SwipeActionButton] {
        ...
        buttons.enumerated().forEach { (index, button) in
            ...
            wrapperViews.append(wrapperView)
        }
        return buttons
    }
    
    var wrapperViews: [SwipeActionButtonWrapperView] = []
    ...
    override func hitTest(_ point: CGPoint, with event: UIEvent?) -> UIView? {
        ...
        let startPoint = width - (minimumButtonWidth * CGFloat(buttons.count))
        let x = point.x - startPoint
        let index = Int(x/minimumButtonWidth)
        
        if index >= 0 && index < buttons.count && point.x > startPoint{
